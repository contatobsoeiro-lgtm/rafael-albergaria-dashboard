...
"""
    ...
    # Substitui getKey() e setMes/setVend se ainda existirem como funções isoladas
    for fn in ["function setMes", "function setVend", "function getKey"]:
        html = re.sub(ref'{fn}\s*\([^)]*\)\s*\{{[\s\S]*?\n\}}', '', html)
...
// ── Gráfico YoY (criado uma vez, atualizado ao entrar em Comparar) ──
let chartYoY = null;
